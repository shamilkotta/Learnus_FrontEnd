...

                {
                    formData.course__content.map((modules, indx)=> (
                        <div className="form__input-group form__module-group" key={indx}>
                            ...
                            <InputText name={`module${indx+1}`} value={modules.module_name} className="form-step2__module-name form__input" placeholder="Module name" minLength="30" showCount maxLength="100" onChange={e=> {handleModuleChange(e, indx)}} required />

                            {
                                modules.chapters.map((chapter, chapterIndx)=> (
                                    <div className="form-module__chapter" key={chapterIndx}>
                                        ...
                                        <InputText name={`course__module-${indx+1}_chapter-${chapterIndx+1}`} value={chapter.chapter_name} className="form-step2-module__chapter-name form__input" minLength="30" maxLength="100" placeholder="Chapter Name" onChange={e=> {handleChapterChange(e, indx ,chapterIndx)}} required />
